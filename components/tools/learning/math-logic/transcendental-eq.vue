<template>
  <div class="math-example-container" ref="transcendentalRootEl"><h3>超越方程数值求解演示</h3>
    <p>
      超越方程是指那些包含超越函数（如三角函数 $\sin(x)$, 指数函数 $e^x$, 对数函数 $\ln(x)$
      等）并且通常不能通过有限次代数运算求得精确解的方程。本演示将通过可视化的方式，展示如何使用经典的数值方法（如牛顿法和二分法）来逼近这些方程的根。
    </p>
    ...
    <div v-if="iterationHistory.length > 0" class="results-section card">
      ...
      <div class="iteration-table-wrapper">
        <table>
          ...
          <tr>
            <th>步骤</th>
            <th>$x_k$</th>
            <th v-if="selectedMethod === 'bisection'">$a_k$</th>
            <th v-if="selectedMethod === 'bisection'">$b_k$</th>
            <th>$f(x_k)$</th>
            <th>$|x_k - x_{k-1}|$ (或区间长度/2)</th>
            <th>状态</th>
          </tr>
          </thead>
          <tbody>
      ...
      <p v-if="currentRoot !== null && iterationHistory.length > 0 && iterationHistory[iterationHistory.length-1].converged && !iterationHistory[iterationHistory.length-1].maxIterReached">
        <strong>最终近似根: $x \approx {{ currentRoot.toFixed(solverParams.displayPrecision) }}$</strong> (在
...
const currentFa = computed(() => {
  if (selectedMethod.value === 'bisection' && currentEquation.value) {
    try {
...
const visualizationTitleHtml = computed(() => {
  ...
  if (eq.g_x_str && eq.h_x_str) {
    // KaTeX 会在父元素上渲染，这里直接插入 LaTeX 字符串
...
function updateExplanation() {
  ...
  if (selectedMethod.value === 'newton') {
    ...
    html += `<p>此方法通过迭代逼近函数 $f(x)$ 的根。迭代公式为：</p>`;
    html += `$$x_{k+1} = x_k - \\frac{f(x_k)}{f'(x_k)}$$`;
  ...
  // KaTeX 渲染将由 explanationHtml 的 watcher 触发
}
...
function setupPlotting() {
  const margin = 40;
  plotScaleX = (canvasWidth.value - 2 * margin) / (xViewRange.value.max - xViewRange.value.min);
  plotScaleY = (canvasHeight.value - 2 * margin) / (yViewRange.value.max - yViewRange.value.min);
...
function drawAxes() {
  ...
  ctx.lineWidth = 0.5;
  const tickSize = 5;
  for (let x = Math.floor(xViewRange.value.min / 0.5) * 0.5; x <= Math.ceil(xViewRange.value.max / 0.5) * 0.5; x += 0.5) {
    const cx = toCanvasX(x);
    ctx.beginPath();
    ctx.moveTo(cx, plotOriginY - tickSize);
    ctx.lineTo(cx, plotOriginY + tickSize);
    ctx.stroke();
    if (Math.abs(x % 1) < 1e-6 || x === 0) {
      ctx.fillStyle = '#666';
      ctx.font = '10px Arial';
      ctx.textAlign = 'center';
      ctx.fillText(x.toFixed(Math.abs(x) < 1e-6 ? 0 : 1), cx, plotOriginY + 15);
    }
  }
  for (let y = Math.floor(yViewRange.value.min / 0.5) * 0.5; y <= Math.ceil(yViewRange.value.max / 0.5) * 0.5; y += 0.5) {
    const cy = toCanvasY(y);
    ctx.beginPath();
    ctx.moveTo(plotOriginX - tickSize, cy);
    ctx.lineTo(plotOriginX + tickSize, cy);
    ctx.stroke();
    if (Math.abs(y % 1) < 1e-6 || y === 0 && Math.abs(plotOriginX) > tickSize * 2) {
      ctx.fillStyle = '#666';
      ctx.font = '10px Arial';
      ctx.textAlign = 'right';
      ctx.fillText(y.toFixed(Math.abs(y) < 1e-6 ? 0 : 1), plotOriginX - 8, cy + 3);
    }
  }
  ctx.strokeStyle = '#333';
  ctx.lineWidth = 1;
...
function plotFunction(funcToPlot, color = 'blue', lineWidth = 2, points = 400) {
  ...
  for (let i = 0; i <= points; i++) {
    ...
    if (cx < 0 || cx > canvasWidth.value || cy < 0 || cy > canvasHeight.value) {
      if (!firstMove) ctx.stroke();
      firstMove = true;
      continue;
    }
...
function drawNewtonStep(xk, fxk, fpxk_val) {
  if (!ctx || fpxk_val === 0 || isNaN(fpxk_val)) return;
...
function redrawCanvas() {
  ...
  if (lastIter) {
    if (selectedMethod.value === 'newton' && lastIter.fpxk !== undefined) {
      drawNewtonStep(lastIter.prev_xk, lastIter.prev_fxk, lastIter.fpxk);
    } else if (selectedMethod.value === 'bisection' && lastIter.ak_prev !== undefined) {
...
async function iterateNewton() {
  ...
  const xk_plus_1 = xk - fxk / fpxk_val;
...
function stepIteration() {
  ...
  method().finally(() => isSolving.value = false);
}


// Renamed @change handler for clarity
function onEquationSelectChange() {
  ...
  nextTick(() => {
    ...
    // Also ensure the visualization title is updated
...
watch([selectedEquationKey, selectedMethod, equationParams], () => {
  ...
  nextTick(() => {
    ...
    if (visualizationSectionEl.value) doKatexRender(visualizationSectionEl.value);
  });
}, {deep: true});
...
.form-item select {
  ...
  /* 让 select 宽度自适应其内容，或者给一个合适的 min-width/width */
  /* KaTeX 渲染后的 option 可能会变宽 */
  min-width: 200px;
  flex-grow: 1;
}
