...
async function setupEnv() {
    ...
    if (dbSetupChoice.toLowerCase() !== 'y') {
        ...
        const suggestedShadowDbName = `${suggestedDbNameBase}_shadow`;
        ...
        console.log('-----------------------------------------');
        console.log('如果您使用的是远程MySQL服务器，请将 \'localhost\' 替换为 \'%\' 或您的应用服务器IP。');
        console.log('重要: 确保用户 \x1b[33m`' + suggestedDbUser + '`\x1b[0m 拥有创建数据库的权限，以便Prisma管理影子数据库，或您需手动创建影子数据库并授权。');

        await askQuestion('\n请在MySQL中执行命令创建主数据库和用户后，按 Enter键 继续...');
        ...
        shadowDbName = `${dbName}_shadow`;
        console.log(`将使用影子数据库名称: ${shadowDbName} (确保用户 '${dbUser}' 有权限让Prisma创建或管理它)`);
    }
    ...
    if (autoGenerateJwt.toLowerCase() !== 'n') {
        ...
        console.log(`已生成新的 JWT_SECRET。`); // 不直接显示密钥
    } else {
        jwtSecret = await askQuestion('请输入您自己的 JWT_SECRET (必须是一个长且随机的字符串):');
        if (!jwtSecret) {
            console.log('JWT_SECRET 未提供，将为您生成一个。');
            jwtSecret = generateJwtSecret();
            console.log(`已生成新的 JWT_SECRET。`);
        }
    }
    envContent = envContent.replace(/^JWT_SECRET=".*?"/m, `JWT_SECRET="${jwtSecret}"`);

    console.log('\n--- 3. DeepSeek AI 助手 API 配置 (可选) ---');
    const setupDeepSeekApi = await askQuestion('您是否需要配置 DeepSeek AI 助手 API 密钥 (用于AI助手功能)? (y/N)', 'N');
    if (setupDeepSeekApi.toLowerCase() === 'y') {
        const deepseekApiKey = await askQuestion('请输入您的 DeepSeek API Key:');
        envContent = envContent.replace(/^DEEPSEEK_API_KEY=".*?"/m, `DEEPSEEK_API_KEY="${deepseekApiKey.trim()}"`);
        console.log('DeepSeek API Key 已配置。');
    } else {
        envContent = envContent.replace(/^DEEPSEEK_API_KEY=".*?"/m, `DEEPSEEK_API_KEY=""`); // 如果跳过，则设为空
        console.log('跳过 DeepSeek API 配置。如果需要此功能，请稍后手动编辑 .env 文件。');
    }

    console.log('\n--- 4. 腾讯翻译 API 配置 (可选) ---');
    const setupTencentApi = await askQuestion('您是否需要配置腾讯翻译 API 密钥 (用于例如菜谱翻译功能)? (y/N)', 'N');
    if (setupTencentApi.toLowerCase() === 'y') {
        ...
        envContent = envContent.replace(/^TENCENT_SECRET_ID=".*?"/m, `TENCENT_SECRET_ID="${tencentSecretId.trim()}"`);
        envContent = envContent.replace(/^TENCENT_SECRET_KEY=".*?"/m, `TENCENT_SECRET_KEY="${tencentSecretKey.trim()}"`);
        envContent = envContent.replace(/^TENCENT_TRANSLATE_REGION=".*?"/m, `TENCENT_TRANSLATE_REGION="${tencentRegion.trim()}"`);
        console.log('腾讯翻译 API 信息已配置。');
    } else {
        envContent = envContent.replace(/^TENCENT_SECRET_ID=".*?"/m, `TENCENT_SECRET_ID=""`);
        envContent = envContent.replace(/^TENCENT_SECRET_KEY=".*?"/m, `TENCENT_SECRET_KEY=""`);
